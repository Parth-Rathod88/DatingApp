...
export class MembersService {
  ...
  getMembers(userParams:UserParams){
    ...
    return getPaginatedResult<Member[]>(this.baseUrl + 'users',params,this.http).pipe(
      map(response => {
        this.memberCache.set(Object.values(userParams).join('-'),response);           //setting key and value of map using memberCache using response from .pipe()
        return response;
      })
    );
  }
  ...
  getMember(username:string){
    //memberCache for single member detail 
